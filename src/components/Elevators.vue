...
<script>
//Elevators number and floors number can be changed
...
export default {
  ...
  data() {
    return {
      WaitingList: [], //queue for waiting floors
      elevatorNumber: numberOfElevators,
      floorNumber: numberOfFloors,
      taskElevator: Array(numberOfElevators).fill(-1), //the floor elevator is sent, -1 of none
      floorArrivalTime: Array(numberOfElevators).fill(0), //calculated arrival time
      elevatorsDirections: Array(numberOfElevators).fill("up"), //helps for animation
      freeElevators: [...Array(numberOfElevators).keys()], //list of free elevator
      floorsNames: ["Ground Floor", "1st", "2nd", "3rd"].concat([...Array(numberOfFloors - 4).keys()].map(x => `${x + 4}th`)),
      elevator_color: Array(numberOfElevators).fill("black"), //"black", "red", "green"
      elevetors_floor: Array(numberOfElevators).fill(0), //where elevators are now
      floor_state: Array(numberOfFloors).fill("Call") // "Call", "Waiting", "Arrived"
    }
  },
  ...
  methods: {
    // get button style
    getVariant(index) {
      const state = this.floor_state[index];
      return state === 'Call' ? 'success' : (state === 'Waiting' ? 'danger' : 'outline-success');
    },

    // Call button click event
    CallElevator(e) {
      const callingFloor = parseInt(e.target.id);
      this.CallElevatorAux(callingFloor);
    },

    // call an elevator to the designed floor
    CallElevatorAux(callingFloor) {
      this.changeFloorState(callingFloor, "Waiting")
      let elevator = this.findClosestFreeElevator(callingFloor);
      if (elevator === undefined) { ////no free elevators
        this.WaitingList.push(callingFloor);
      }
      else {
        this.taskElevator[elevator] = callingFloor;
        this.elevator_color[elevator] = "red"
        this.nextFloor(elevator);
      }
    },

    //takes the elevator to the next floor
    nextFloor(elevator) {
      if (this.elevetors_floor[elevator] === this.taskElevator[elevator]) { //elevator has arrived
        this.changeFloorState(this.elevetors_floor[elevator], "Arrived")
        this.elevator_color[elevator] = "green"
        this.playSound()
        setTimeout(() => { // 2s wait
          this.changeFloorState(this.elevetors_floor[elevator], "Call")
          this.elevator_color[elevator] = "black"
        ...
        let copy = [...this.elevetors_floor]
        copy[elevator] -= Math.sign(this.elevetors_floor[elevator] - this.taskElevator[elevator]);
        const floorsDiff = Math.abs(this.elevetors_floor[elevator] - this.taskElevator[elevator]);
        this.floorArrivalTime[elevator] = floorsDiff;
        this.elevatorsDirections[elevator] = (this.elevetors_floor[elevator] - this.taskElevator[elevator]) > 0 ? 'down' : 'up';
        this.elevetors_floor = copy;
        setTimeout(() => {
          this.nextFloor(elevator)
        }, 1000);
      }
    ...
    changeFloorState(floor, state) {
      let copy = [...this.floor_state];
      copy[floor] = state;
      this.floor_state = copy;
    },

    findClosestFreeElevator(floor) {
      const posibleElevators = this.freeElevators.sort((a, b) =>
      Math.abs(this.elevetors_floor[a] - floor) - Math.abs(this.elevetors_floor[b] - floor));
    ...
    playSound() {
      var audio = new Audio(require('../assets/bell_small_002.mp3'))
      audio.play();
